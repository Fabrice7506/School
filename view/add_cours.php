...
<?php
    require('../Database/db.php');
      ...
      if(isset($_POST['Send'])){
          if( !empty($_POST['Mat'])){
            try{
              $Verify_teacher = $connexion -> prepare('SELECT * FROM cours WHERE Nom_cours = ? ');
              $Verify_teacher -> execute(array(strtolower($_POST['Mat'])));
              $resultat = $Verify_teacher -> fetchAll();
              $Verify_teacher -> closeCursor();
            }catch(PDOException $e){
              $UserExist =  "cet Matière existe deja";
            }
  
              if(empty($resultat)){
                  $add_teacher = $connexion ->prepare('INSERT INTO cours(Nom_cours) VALUES ( ?  )');
                  $add_teacher -> execute(array(strtolower($_POST['Mat'])));
...
?>
    ...
    <form method="post">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id=""  name="Mat"  placeholder="Entrez votre Nom...">
            </div>
